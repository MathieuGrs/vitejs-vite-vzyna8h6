...
import {
  ...
  Send, Loader, Clock, ChevronLeft, ChevronRight, LogOut
} from 'lucide-react';
...
// --- UTILITAIRE : CONVERSION HEX VERS RGBA (Corrige le bug du fond noir) ---
...
export default function App() {
  ...
  // Active API Key
  const [activeApiKey, setActiveApiKey] = useState(localStorage.getItem('geminiApiKey') || '');
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', text: "Coucou ! 👋 Je suis Emeth. N'hésitez pas à me demander de créer ou valider vos tâches !" }
  ]);
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      // Sauvegarder la clé API pour les prochaines fois
  ...
  const toggleSubtaskCompletion = async (taskId, subtaskId) => {
    ...
    await updateDoc(doc(db, 'users', user.uid, 'tasks', taskId), { subtasks: updatedSubtasks });
  };
  ...
  const handleTimeChange = (e) => {
    ...
    if (val.length > 2) setNewTaskTime(val.substring(0, 2) + ':' + val.substring(2)); else setNewTaskTime(val);
  };

  // --- ASSISTANT GEMINI ---
  const callGeminiWithRetry = async (prompt, retries = 5) => {
    let delay = 1000;
    const activeTasksStr = tasks.filter(t => !t.completed).map(t => `- ID: ${t.id} | Titre: ${t.title} (Date: ${t.date || 'Non définie'}, Catégorie: ${categories.find(c => c.id === t.categoryId)?.name || 'Aucune'})`).join('\n');
    const categoriesStr = categories.map(c => `- ID: ${c.id} | Nom: ${c.name}`).join('\n');
    const now = new Date();

    const systemPrompt = `Tu es Emeth, un petit robot assistant de productivité joyeux et ultra-efficace. Ton mot d'ordre : la concision. Tes réponses doivent être extrêmement synthétiques, directes et percutantes (2 à 3 phrases maximum), tout en gardant une attitude très positive.
    
Aujourd'hui, nous sommes le ${now.toLocaleDateString('fr-FR')} et il est ${now.toLocaleTimeString('fr-FR')}.

IMPORTANT : Tu as la capacité d'agir sur l'application. Si l'utilisateur te demande de créer, valider ou supprimer une tâche, tu DOIS inclure un bloc JSON pur dans ta réponse pour exécuter l'action. 
Tu peux aussi déduire et créer automatiquement des sous-missions. Par exemple, si l'utilisateur dit "séance de sport pour dos et biceps", tu dois créer la tâche "Séance de sport" et y inclure les sous-missions ["Dos", "Biceps"].

CRUCIAL : Lorsque tu crées une tâche, tu dois analyser son sujet et DEVINER dans quelle catégorie elle doit aller. Utilise null pour "categoryId" si aucune ne correspond.

Actions JSON acceptées :
1. Créer : {"action": "create_task", "title": "Nom", "date": "YYYY-MM-DD", "time": "HH:MM", "categoryId": "ID_CATEGORIE_OU_null", "subtasks": ["Sous-mission 1"]}
2. Valider : {"action": "complete_task", "taskId": "ID_TACHE"}
3. Supprimer : {"action": "delete_task", "taskId": "ID_TACHE"}

Catégories disponibles :
${categoriesStr || "Aucune catégorie existante. Utilise toujours null."}

Tâches actuelles :
${activeTasksStr || "Aucune tâche en cours."}

Réponds en français avec des emojis. Place le JSON à la toute fin.`;

    const payload = { contents: [{ parts: [{ text: prompt }] }], systemInstruction: { parts: [{ text: systemPrompt }] } };
    const GEMINI_API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${activeApiKey}`;
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        return data.candidates?.[0]?.content?.parts?.[0]?.text || "Mince, je n'ai pas de réponse à formuler.";
      } catch (error) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const responseText = await callGeminiWithRetry(userText);
      ...
      if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
        ...
        if (possibleJson.includes('"action"')) {
          try {
            actionData = JSON.parse(possibleJson);
            cleanedText = responseText.substring(0, firstBrace).trim() + ' ' + responseText.substring(lastBrace + 1).trim();
            cleanedText = cleanedText.replace(/```json/g, '').replace(/```/g, '').trim();
          } catch(err) {}
        }
      }

      setMessages(prev => [...prev, { role: 'assistant', text: cleanedText || "Mission accomplie ! 🚀" }]);

      if (actionData && user) {
        if (actionData.action === 'create_task') {
          let initialSubtasks = (actionData.subtasks && Array.isArray(actionData.subtasks)) ? actionData.subtasks.map(st => ({ id: crypto.randomUUID(), title: st, completed: false })) : [];
          const assignedCategoryId = (actionData.categoryId && actionData.categoryId !== "null") ? actionData.categoryId : null;
          await addDoc(collection(db, 'users', user.uid, 'tasks'), {
            title: actionData.title || "Nouvelle tâche", date: actionData.date || null, time: actionData.time || null,
            categoryId: assignedCategoryId, completed: false, subtasks: initialSubtasks, createdAt: new Date().toISOString()
          });
        } else if (actionData.action === 'complete_task' && actionData.taskId) {
          await updateDoc(doc(db, 'users', user.uid, 'tasks', actionData.taskId), { completed: true });
        } else if (actionData.action === 'delete_task' && actionData.taskId) {
          await deleteDoc(doc(db, 'users', user.uid, 'tasks', actionData.taskId));
        }
      }
    } catch (error) {
      setMessages(prev => [...prev, { role: 'assistant', text: "Désolé, j'ai eu un problème de connexion (ou votre clé API est invalide). 😴" }]);
  ...
  // --- ECRANS D'AFFICHAGE ---
  if (loadingAuth) {
    return <div className="min-h-screen flex items-center justify-center bg-slate-50"><Loader className="w-12 h-12 text-purple-500 animate-spin" /></div>;
  }

  // --- ECRAN DE CONNEXION AVEC CLÉ API ---
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-4">
        <div className="bg-white p-8 rounded-[2rem] shadow-xl max-w-sm w-full text-center border border-white">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            <input type="email" placeholder="Votre Email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition-all" required />
            <input type="password" placeholder="Mot de passe (6 carac. min)" value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition-all" required />
            
            {/* CHAMP POUR LA CLÉ API */}
            <div className="pt-3 border-t border-slate-100 mt-2">
              <p className="text-xs font-bold text-purple-500 mb-2">🧠 L'assistant Emeth</p>
              <input type="text" placeholder="Collez votre clé API Gemini (Optionnel)" value={apiKeyInput} onChange={e=>setApiKeyInput(e.target.value)} className="w-full bg-purple-50 border border-purple-100 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-200 transition-all" />
              <p className="text-[9px] text-slate-400 mt-1.5 leading-tight">Laissez vide si vous ne souhaitez pas utiliser le robot. La clé est sauvegardée localement.</p>
            </div>

            {authError && <p className="text-red-500 text-xs font-bold mt-2">{authError}</p>}
            <button type="submit" className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg hover:scale-[1.02] mt-4">
              {isRegistering ? "Créer mon compte" : "Se connecter"}
            </button>
          </form>
          <button onClick={() => {setIsRegistering(!isRegistering); setAuthError('');}} className="mt-5 text-xs font-bold text-slate-400 hover:text-purple-600 transition-colors">
    ...
    return (
      <div className="w-full sm:w-[250px]">
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 flex gap-2">
          <button type="button" onClick={(e) => {e.stopPropagation(); setNewTaskDate(''); setIsDatePickerOpen(false)}} className="flex-1 text-sm sm:text-xs font-bold text-slate-400 py-2.5 sm:py-2 bg-slate-50 rounded-xl">Effacer</button>
          <button type="button" onClick={(e) => {e.stopPropagation(); setNewTaskDate(new Date().toISOString().split('T')[0]); setIsDatePickerOpen(false)}} className="flex-1 text-sm sm:text-xs font-bold text-purple-600 py-2.5 sm:py-2 bg-purple-50 rounded-xl">Aujourd'hui</button>
        </div>
      </div>
    );
  };
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] p-4 sm:p-6 shadow-xl shadow-indigo-100/50 border border-white max-w-6xl mx-auto flex flex-col h-[75vh]">
        <div className="flex flex-col sm:flex-row items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto justify-between sm:justify-start">
            <button onClick={() => setCalendarMonth(new Date(year, month - 1, 1))} className="p-2 bg-white rounded-xl shadow-sm text-slate-600"><ChevronLeft className="w-5 h-5"/></button>
            <button onClick={() => setCalendarMonth(new Date())} className="px-4 py-2 flex-1 sm:flex-none bg-white rounded-xl shadow-sm text-sm font-bold text-slate-600">Aujourd'hui</button>
            <button onClick={() => setCalendarMonth(new Date(year, month + 1, 1))} className="p-2 bg-white rounded-xl shadow-sm text-slate-600"><ChevronRight className="w-5 h-5"/></button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1 sm:gap-2 flex-1 overflow-y-auto no-scrollbar pb-4">
          {days.map((day, idx) => {
            ...
            return (
              <div key={day} className={`bg-white rounded-xl sm:rounded-2xl p-1 sm:p-2 min-h-[60px] sm:min-h-[100px] flex flex-col border-2 transition-all ${isToday ? 'border-purple-400 shadow-md' : 'border-transparent hover:border-purple-100'}`}>
  ...
  return (
    <div 
      className="min-h-screen font-sans flex flex-col overflow-hidden selection:bg-purple-200 transition-all duration-1000 ease-in-out"
      style={{ 
        /* UTILISATION DU NOUVEAU TRADUCTEUR RGBA ICI */
        background: `radial-gradient(circle at top left, rgba(255,255,255,0.95) 0%, ${hexToRgba(activeColor, 0.2)} 100%)`, 
    ...
    >
      ...
      <header className="px-4 sm:px-6 py-4 flex items-center justify-between shrink-0 bg-white/40 backdrop-blur-md z-10 relative">
        <div className="flex items-center gap-2 sm:gap-3 text-purple-600">
          <div className="flex items-center justify-center rounded-full drop-shadow-sm">
            <EmethHead className="w-10 h-10 sm:w-14 sm:h-14" />
          </div>
          <h1 className="text-xl sm:text-2xl font-black tracking-tight hidden sm:block">Flow</h1>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => signOut(auth)} className="w-10 h-10 bg-white hover:bg-red-50 border border-slate-200 hover:border-red-200 text-slate-400 hover:text-red-500 rounded-full flex items-center justify-center transition-all shadow-sm" title="Se déconnecter">
      ...
      <nav className="flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-3 overflow-x-auto no-scrollbar bg-white/30 backdrop-blur-md border-b border-white/50 shadow-sm shrink-0 relative z-10">
        <button onClick={() => setCurrentFilter('all')} className={`px-4 py-2 sm:py-2.5 rounded-2xl text-sm font-bold whitespace-nowrap transition-all flex items-center gap-2 shrink-0 ${currentFilter === 'all' ? 'bg-slate-800 text-white shadow-lg scale-105' : 'bg-white/80 text-slate-600 hover:bg-white hover:shadow-sm'}`}>
          <List className="w-4 h-4"/> Toutes
        </button>

        {categories.map(cat => (
          <div key={cat.id} className="relative group shrink-0">
            <button onClick={() => setCurrentFilter(cat.id)} style={{ backgroundColor: currentFilter === cat.id ? cat.color : `${cat.color}15`, color: currentFilter === cat.id ? '#ffffff' : cat.color, borderColor: currentFilter === cat.id ? 'transparent' : cat.color }} className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-2xl font-bold text-sm border-2 whitespace-nowrap transition-all duration-300 flex items-center gap-2 ${currentFilter === cat.id ? 'shadow-xl scale-105' : 'hover:scale-105'}`}>
              {cat.name}
            </button>
            <button onClick={(e) => { e.stopPropagation(); handleDeleteCategory(cat.id); }} className="absolute -top-2 -right-2 bg-red-500 text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-md z-10 hover:bg-red-600 hover:scale-110"><Trash className="w-3.5 h-3.5" /></button>
          </div>
        ))}
        ...
        <button onClick={() => setCurrentFilter('today')} className={`px-3 py-2 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all flex items-center gap-1.5 shrink-0 ${currentFilter === 'today' ? 'bg-blue-500 text-white shadow-md scale-105' : 'bg-white/60 text-slate-500 hover:bg-white'}`}><Clock className="w-3.5 h-3.5"/> Aujourd'hui</button>
        <button onClick={() => setCurrentFilter('upcoming')} className={`px-3 py-2 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all flex items-center gap-1.5 shrink-0 ${currentFilter === 'upcoming' ? 'bg-orange-500 text-white shadow-md scale-105' : 'bg-white/60 text-slate-500 hover:bg-white'}`}><Calendar className="w-3.5 h-3.5"/> À venir</button>
        <button onClick={() => setCurrentFilter('calendar')} className={`px-3 py-2 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all flex items-center gap-1.5 shrink-0 ${currentFilter === 'calendar' ? 'bg-purple-500 text-white shadow-md scale-105' : 'bg-white/60 text-slate-500 hover:bg-white'}`}><Calendar className="w-3.5 h-3.5"/> Calendrier</button>
      </nav>
            ...
            <form onSubmit={handleCreateTask} className="bg-white/90 backdrop-blur-xl rounded-[1.5rem] sm:rounded-[2rem] p-2 shadow-xl shadow-indigo-100/50 border border-white flex flex-col sm:flex-row items-center gap-2 max-w-5xl mx-auto relative z-20">
              ...
              <div className="flex w-full sm:w-auto items-center justify-between sm:justify-end gap-1.5 sm:gap-2">
                <div className="relative shrink-0 flex-1 sm:flex-none">
                  <button type="button" onClick={() => setIsDatePickerOpen(true)} className="flex items-center justify-center sm:justify-between bg-slate-50/80 hover:bg-white border border-slate-200 rounded-xl sm:rounded-2xl px-2 sm:px-4 py-2.5 w-full sm:w-[130px] transition-all">
                  ...
                  {isDatePickerOpen && (
                    <>
                      ...
                      <div className="absolute top-full mt-3 right-0 sm:left-1/2 sm:-translate-x-1/2 p-4 bg-white rounded-3xl shadow-2xl border border-slate-100 z-[70] w-auto">
                        {renderMiniCalendar()}
                      </div>
                    </>
                  )}
                ...
                <div className="flex flex-1 sm:flex-none items-center justify-center sm:justify-start bg-slate-50/80 hover:bg-white border border-slate-200 rounded-xl sm:rounded-2xl px-2 sm:px-4 py-2.5 w-full sm:w-[100px] shrink-0 focus-within:ring-2 focus-within:ring-purple-400 transition-all">
                  <Clock className="w-4 h-4 text-slate-400 shrink-0 sm:mr-2" />
                  <input 
                    type="text" inputMode="numeric" pattern="[0-9]*" placeholder="12:00" maxLength="5"
                    ...
                    value={newTaskTime} onChange={handleTimeChange}
                  />
                </div>

                <select className="flex-1 sm:flex-none bg-slate-50/80 hover:bg-white border border-slate-200 rounded-xl sm:rounded-2xl px-2 sm:px-4 py-2.5 text-xs sm:text-sm font-bold outline-none w-full sm:w-[130px] shrink-0 truncate cursor-pointer transition-all" value={newTaskCategoryId} onChange={(e) => setNewTaskCategoryId(e.target.value)} style={{ color: newTaskCategoryId ? getCategoryColor(newTaskCategoryId) : '#64748B' }}>
                ...
                <button type="submit" disabled={!newTaskTitle.trim()} className="bg-gradient-to-r from-purple-500 to-pink-500 text-white w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-full shadow-md disabled:opacity-50 flex items-center justify-center shrink-0 hover:scale-105 transition-transform">
              ...
              {filteredTasks.length === 0 ? (
                <div className="text-center py-16 sm:py-20 text-slate-400">
                  <div className="w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center mx-auto mb-4 drop-shadow-md">
                    <RoboticArm className="w-full h-full text-purple-400" />
                  </div>
                ...
                filteredTasks.map(task => (
                  <div key={task.id} className={`bg-white/90 backdrop-blur-md rounded-[1.5rem] sm:rounded-[2rem] border border-white shadow-lg transition-all duration-300 ${task.completed ? 'opacity-60 shadow-none' : 'shadow-slate-200/50 hover:shadow-xl'}`}>
                    <div className="p-4 flex items-start gap-3 sm:gap-4">
                      <button onClick={() => toggleTaskCompletion(task)} className={`mt-0.5 sm:mt-1 rounded-full flex-shrink-0 transition-colors ${task.completed ? 'text-green-500' : 'text-slate-300 hover:text-purple-400'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 shrink-0">
                            {task.date && !isNaN(new Date(task.date).getTime()) && (
                              <span className="flex items-center gap-1 text-[10px] sm:text-xs font-bold bg-slate-100 text-slate-600 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl">
                                <Calendar className="w-3 h-3" />
                                {new Date(task.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}
                                {task.time && <span className="ml-1 text-purple-500 border-l border-slate-300 pl-1">{task.time}</span>}
                              </span>
                            )}
                            <div className="relative flex items-center group/cat hover:scale-105 transition-transform shrink-0">
                              ...
                              <select value={task.categoryId || ""} onChange={(e) => updateTaskCategory(task.id, e.target.value)} className={`appearance-none pl-6 pr-6 py-1 sm:py-1.5 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold w-[90px] sm:w-[110px] truncate transition-all cursor-pointer outline-none focus:ring-2 focus:ring-purple-400 ${task.categoryId ? 'text-white' : 'text-slate-600 bg-slate-100 hover:bg-slate-200'}`} style={task.categoryId ? { backgroundColor: getCategoryColor(task.categoryId) } : {}}>
                        ...
                        <div className="mt-2 sm:mt-3 flex items-center justify-between">
                          <button onClick={() => toggleExpandTask(task.id)} className="flex items-center gap-1 text-xs sm:text-sm font-bold text-slate-400 hover:text-purple-500 transition-colors bg-slate-50 px-2 sm:px-3 py-1.5 rounded-lg sm:rounded-xl">
                      ...
                      <div className="px-4 pb-4 pt-1 border-t border-slate-100/50 bg-slate-50/50 rounded-b-[1.5rem] sm:rounded-b-[2rem]">
                        <div className="space-y-2 mb-3 pl-9 sm:pl-11 mt-2">
                          {(task.subtasks || []).map(sub => (
                            <div key={sub.id} className="flex items-center gap-2 sm:gap-3 group">
                              <button onClick={() => toggleSubtaskCompletion(task.id, sub.id)} className={`transition-colors ${sub.completed ? 'text-green-500' : 'text-slate-300 hover:text-purple-400'}`}>
                                {sub.completed ? <Check className="w-4 h-4 sm:w-5 sm:h-5" /> : <Circle className="w-4 h-4 sm:w-5 sm:h-5" />}
                              </button>
                              <span className={`text-xs sm:text-sm flex-1 ${sub.completed ? 'text-slate-400 line-through' : 'text-slate-700 font-medium'}`}>{sub.title}</span>
                              <button onClick={() => deleteSubtask(task.id, sub.id)} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-red-500 p-1.5 rounded-full hover:bg-red-50 transition-all"><X className="w-3.5 h-3.5" /></button>
                            </div>
                          ))}
                        </div>
                        <div className="pl-9 sm:pl-11 flex items-center gap-2">
                          <Plus className="w-3.5 h-3.5 text-slate-400" />
                          <input type="text" placeholder="Ajouter une étape..." className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs sm:text-sm text-slate-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-300 transition-all" onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddSubtask(task.id, e.target.value); e.target.value = ''; } }} />
                        </div>
                      </div>
                    )}
                  </div>
      ...
      {/* --- ROBOT FLOTTANT (Apparaît UNIQUEMENT si une clé API est entrée) --- */}
      {activeApiKey && !isAssistantOpen && (
        <button onClick={() => setIsAssistantOpen(true)} className="fixed bottom-6 right-4 sm:right-6 lg:bottom-10 lg:right-10 z-30 group bot-animate cursor-pointer focus:outline-none">
          <div className="relative">
            ...
            <svg width="70" height="70" viewBox="0 0 100 100" className="drop-shadow-2xl sm:w-[80px] sm:h-[80px] hover:scale-110 transition-transform duration-300">
      ...
      <div className={`fixed inset-y-0 right-0 z-50 w-full sm:w-[400px] bg-white sm:bg-white/95 sm:backdrop-blur-3xl shadow-2xl transform transition-transform duration-500 flex flex-col ${isAssistantOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-purple-50 to-pink-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-purple-600 font-bold">Votre assistant</p>
            </div>
          </div>
          <button onClick={() => setIsAssistantOpen(false)} className="p-2 bg-white rounded-full text-slate-400 hover:text-slate-700 shadow-sm transition-transform hover:scale-110"><X className="w-5 h-5" /></button>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] p-3 sm:p-4 rounded-2xl sm:rounded-3xl text-sm shadow-sm ${msg.role === 'user' ? 'bg-gradient-to-br from-purple-500 to-pink-500 text-white rounded-tr-sm' : 'bg-white text-slate-700 border border-slate-100 rounded-tl-sm'}`}>
          ...
          {isTyping && (
            <div className="flex justify-start">
              <div className="bg-white p-3 rounded-2xl rounded-tl-sm border border-slate-100 flex items-center gap-2"><Loader className="w-4 h-4 animate-spin text-purple-400" /></div>
            </div>
          )}
        ...
        <div className="p-3 sm:p-4 bg-white border-t border-slate-100 pb-safe">
          <form onSubmit={handleSendMessage} className="flex items-center gap-2 bg-slate-100/80 p-1.5 sm:p-2 rounded-full border border-slate-200 focus-within:border-purple-300 focus-within:bg-white transition-all">
            ...
            <button type="submit" disabled={!inputMessage.trim() || isTyping} className="p-2 sm:p-2.5 bg-purple-500 hover:bg-purple-600 text-white rounded-full disabled:opacity-50 transition-all shadow-sm"><Send className="w-4 h-4 ml-0.5" /></button>
          </form>
        </div>
      </div>
      ...
      {isCategoryModalOpen && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl transform transition-all">
            <button onClick={() => setIsCategoryModalOpen(false)} className="absolute top-4 right-4 p-2 text-slate-400 hover:bg-slate-100 rounded-full transition-colors"><X className="w-5 h-5" /></button>
            <div className="flex items-center gap-3 mb-6">
              <div className="bg-purple-100 p-2.5 rounded-xl text-purple-600"><Palette className="w-5 h-5" /></div>
            ...
            <form onSubmit={handleCreateCategory} className="space-y-5">
              <div>
                ...
                <input type="text" autoFocus placeholder="Ex: Perso, Pro..." className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 transition-all" value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-600 mb-2">Couleur</label>
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  {CATEGORY_COLORS.map(color => <button key={color} type="button" onClick={() => setNewCategoryColor(color)} className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full transition-all shadow-sm ${newCategoryColor === color ? 'scale-125 ring-4 ring-offset-2 ring-purple-200' : 'hover:scale-110'}`} style={{ backgroundColor: color }} />)}
                </div>
              </div>
              <button type="submit" disabled={!newCategoryName.trim()} className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-3.5 rounded-xl mt-2 transition-all disabled:opacity-50 shadow-lg">Créer</button>
            </form>
          </div>
        </div>
      )}
